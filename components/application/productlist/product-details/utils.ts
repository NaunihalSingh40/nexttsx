type CustomizationItem = {
  ...
  type CustomizationState = Record<number, any>; // Adjust 'any' based on actual usage
  
  export const formatCustomizations = (customisation_items: any[]): CustomizationItem[] => {
    const customizations: CustomizationItem[] = customisation_items?.map((customization) => {
      ...
      const isDefaultTag = parentTag.list.find((tag: { code: string }) => tag.code === "default");
      const isDefault = isDefaultTag?.value.toLowerCase() === "yes";
      const childTag = itemDetails.tags.find((tag: { code: string }) => tag.code === "child");
      const childs = childTag?.list.map((item: { value: string }) => item.value);
  ...
  export const formatCustomizationGroups = (customisation_groups: any[]): CustomizationGroup[] => {
    const formattedCustomizationGroups: CustomizationGroup[] = customisation_groups?.map((group) => {
      const configTags = group.tags.find((tag: { code: string }) => tag.code === "config").list;
      const minConfig = configTags.find((tag: { code: string }) => tag.code === "min").value;
      const maxConfig = configTags.find((tag: { code: string }) => tag.code === "max").value;
      const inputTypeConfig = configTags.find(
        (tag: { code: string }) => tag.code === "input"
      ).value;
      const seqConfig = configTags.find((tag: { code: string }) => tag.code === "seq").value;
  
      const customizationObj: CustomizationGroup = {
        id: group.local_id,
  ...
  export const getCustomizationGroupsForProduct = (allGroups: CustomizationGroup[], ids: string[]): CustomizationGroup[] => {
    return allGroups.filter((g) => {
      return ids.includes(g.id);
    });
  };
  
  export const hasCustomizations = (productPayload: any): boolean => {
    return productPayload.item_details.tags.find(
      (item: { code: string }) => item.code === "custom_group"
    )
      ? true
      : false;
  };
  ...
  export const initializeCustomizationState_ = async (
    ...
    if (firstGroup) {
      ...
      let level = 1;
      const newState: CustomizationState = { ...customization_state };
  
      while (currentGroup) {
        const group = customizationGroups.find(
  ...
  export const createCustomizationAndGroupMapping = (customizations: CustomizationItem[]) => {
    let newCustomizationGroupMappings: Record<string, Set<string>> = {};
    let customizationToGroupMap: Record<string, string[]> = {};
    ...
    const finalizedCustomizationGroupMappings: Record<string, string[]> = {};
  ...
  export function findMinMaxSeq(customizationGroups: CustomizationGroup[]): { minSeq: number | undefined, maxSeq: number | undefined } {
  ...
  export const findSelectedCustomizationForGroup = (
    group: any,
    childCustomizations: CustomizationItem[]
  ): CustomizationItem[] => {
  ...
  export function areCustomisationsSame(existingIds: string[], currentIds: string[]): boolean {
